...
function updatePreference(cartItems) {
    axios.post('/mercadopago/preferencia', { cart_items: cartItems })
        .then(response => {
            const preferenceId = response.data.preferenceId;

            // Actualizar el widget de Mercado Pago
        ...
        .catch(error => console.error('Error al actualizar preferencia:', error));
}

// Simular carrito de compras dinámico
const cartItems = [
    { title: "Producto 1", quantity: 1, unit_price: 2000 },
    { title: "Producto 2", quantity: 2, unit_price: 1500 },
];

// Llama a esta función cada vez que cambie el carrito
updatePreference(cartItems);
